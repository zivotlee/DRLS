...
class Cluster(object):
    ...
    def pod_random_placement(self, job):    
        '''
        updating
        '''
        ...
        if ('multi_gpu' in job) and (job['multi_gpu'] == 'horovod'):
            num_pack = job['packing']
            num_full_nodes = math.floor(num_w / num_pack)
            last_node_gpu = num_w % num_pack
            num_node = num_full_nodes
            num_gpu_list = list([num_pack] * int(num_full_nodes))
            if last_node_gpu != 0:
                num_node += 1
                num_gpu_list.append(last_node_gpu)
            num_node = int(num_node)      
